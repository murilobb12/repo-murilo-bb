...
function startCurio() {
  ...
  docker run -d --network host --env-file "$PWD"/.env_curio --name CURIO --rm atf.intranet.bb.com.br:5001/bb/iib/iib-curio:0.7.0
}
...
function executaModoLocal() {
  ...
  if [[ $1 == "true" ]]; then
    ./mvnw compile quarkus:dev -DQUARKUS_DATASOURCE_USERNAME=$USER_DB2 -DQUARKUS_DATASOURCE_PASSWORD=$PASSWORD_DB2 -Duser.language=pt -Duser.region=BR
  else
    ./mvnw compile quarkus:dev -Duser.language=pt -Duser.region=BR
  fi
}
